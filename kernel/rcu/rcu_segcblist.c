...
/*
 * Clean up and compact the segmented callback list after callbacks have
 * been advanced to the RCU_DONE_TAIL segment.  The @i parameter is the
 * index of the first segment that was NOT advanced (i.e., the segment
 * after the last one moved to RCU_DONE_TAIL).  This function fixes up
 * tail pointers and compacts any gaps left by the moved segments.
 */
...
/*
 * SRCU wrapper for rcu_segcblist_advance() - SRCU only uses normal GP
 * sequences and doesn't track expedited GPs separately.  This compares
 * the SRCU GP sequence directly against rgos_norm, rather than calling
 * poll_state_synchronize_rcu_full() which checks RCU-specific globals.
 */
