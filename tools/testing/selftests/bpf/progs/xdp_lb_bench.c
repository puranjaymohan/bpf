...
int xdp_lb_bench(struct xdp_md *xdp)
{
	...
	/* Increment once per batch; applied per-iteration in reset block */
	if (cold_lru) {
		__u32 *saddr = data + saddr_off;

		batch_gen++;
		if ((void *)(saddr + 1) <= data_end)
			*saddr ^= batch_gen * KNUTH_HASH_MULT;
	}
	...
	return BENCH_BPF_LOOP(
		...
		({
			...
			/*
			 * XOR per-batch hash into source so the flow key
			 * differs between batches.  Re-read data/data_end
			 * since bpf_xdp_adjust_head invalidates them.
			 */
			if (cold_lru) {
				...
				if ((void *)(__sa + 1) <= de)
					*__sa ^= batch_gen * KNUTH_HASH_MULT;
			}
		})
	);
}
