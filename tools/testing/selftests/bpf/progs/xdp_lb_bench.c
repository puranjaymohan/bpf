...
/*
 * XDP load-balancer benchmark: a simplified L4 load-balancer inspired by
 * katran.  Unified IPv4/IPv6 pipeline with in-BPF batch timing.
 *
 * Pipeline: parse L3/L4 -> VIP lookup -> LRU / consistent-hash ->
 *           real lookup -> per-VIP & per-real stats -> IPIP/IP6IP6 encap
 */
...
__u64 __attribute__((__aligned__(256))) timing_samples[NR_CPUS][NR_SAMPLES];
__u32 __attribute__((__aligned__(256))) timing_idx[NR_CPUS];

const volatile __u32 batch_iters = 1000;
volatile __u32 timing_enabled;
volatile __u32 flow_mask;
volatile __u32 cold_lru;
__u32 batch_gen;

static __always_inline __be32 create_encap_ipv4_src(__u16 port, __be32 src)
{
...
static __always_inline void create_encap_ipv6_src(__u16 port, __be32 src, __be32 *saddr)
{
...
/* Check new-connection rate.  Always performs a stats lookup + ktime call. */
...
static __noinline int process_packet(struct xdp_md *xdp)
{
	...
	/* Mix batch generation into flow key to force LRU misses */
	if (batch_gen)
		flow.srcv6[0] ^= batch_gen * KNUTH_HASH_MULT;
...
{
	__u32 cpu = bpf_get_smp_processor_id() & CPU_MASK;
	void *data     = (void *)(long)xdp->data;
	void *data_end = (void *)(long)xdp->data_end;
	struct ethhdr *eth = data;
	struct ethhdr saved_eth;
	__u64 batch_start, elapsed;
	__u32 rand_state = 0;
	int saddr_off = 0;
	bool is_v6;
	int result;
	...
	batch_start = bpf_ktime_get_ns();

	bpf_repeat(batch_iters - 1) {
		result = process_packet(xdp);
	...
	elapsed = bpf_ktime_get_ns() - batch_start;

	if (timing_enabled) {
		__u32 idx = timing_idx[cpu] % NR_SAMPLES;

		timing_samples[cpu][idx] = elapsed;
		timing_idx[cpu] += 1;
	}

	/* Untimed final iteration leaves packet encapsulated for validation */
	return process_packet(xdp);
}
