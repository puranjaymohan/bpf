...
static const struct test_scenario scenarios[NUM_SCENARIOS] = {
	...
	[S_TCP_V4_LRU_WARMUP] = {
		...
		.description       = "IPv4 TCP, 4K flows, ~50% LRU miss (fixed batch_iters=6500)",
		.expected_retval   = XDP_TX,
		.expect_encap      = true,
		.vip_addr          = IP4(10, 10, 1, 1),
		.dst_port          = 80,
	...
	[S_PASS_V6_NO_VIP] = {
		...
		.expected_retval = XDP_PASS,
		.is_v6           = true,
		.vip_addr_v6     = IP6(0xfd009900, 0, 0, 1),
		.dst_port        = 80,
		.ip_proto        = IPPROTO_TCP,
		.src_addr_v6     = IP6(0xfd000400, 0, 0, 1),
		.src_port        = 33333,
	},
	[S_PASS_V4_ICMP] = {
		.name            = "pass-v4-icmp",
	...
	[S_DROP_V6_FRAG] = {
		...
		.dst_port        = 80,
		.ip_proto        = IPPROTO_TCP,
		.src_addr_v6     = IP6(0xfd000500, 0, 0, 1),
		.src_port        = 44444,
		.set_frag        = true,
	},
};
...
/*
 * Build the expected output packet for a scenario.
 * For XDP_TX (encap): outer Eth + outer IP/IPv6 + inner packet (sans Eth).
 * For XDP_PASS: output == input.
 * For XDP_DROP: no output to validate.
 */
static void build_expected_packet(int idx)
{
	...
	/* XDP_TX: build encapsulated packet */

	/* Outer Ethernet: dst=router_mac, src=original dst mac (lb_mac) */
	{
		struct ethhdr eth = {};
	...
	/* Inner packet: original packet minus Ethernet header */
			   ...
			   const __u8 *exp, __u32 exp_len)
{
	...
	fprintf(stderr, "  [%s] hex diff (got %u bytes, expected %u bytes):\n",
		name, got_len, exp_len);
	fprintf(stderr, "  offset  got                               expected\n");

	for (i = 0; i < max_len; i += 16) {
		bool differs = false;
		__u32 j;

		for (j = i; j < i + 16 && j < max_len; j++) {
			if (j >= got_len || j >= exp_len ||
			    got[j] != exp[j]) {
				differs = true;
				break;
			}
...
{
	struct lb_stats values[NR_CPUS];
	unsigned int nr_cpus = bpf_num_possible_cpus();
	__u64 v1 = 0, v2 = 0;
	unsigned int i;
...
static bool validate_counters(int idx)
{
	...
	/* For encap scenarios: verify LRU packet count */
	if (lru_pkts != n) {
	...
	/*
	 * Expected LRU misses depend on the scenario:
	 *  - prepopulate_lru (no SYN): 0 (all hits)
	 *  - SYN / F_LRU_BYPASS / RST: n (every iteration goes to CH)
	 *  - cold_lru without flow_mask: 1 (first poisoned miss, then hits)
	 *
	 * flow_mask is disabled during validation, so cold_lru scenarios
	 * only miss on the first iteration (poison → miss → insert → hits).
	 * RST prevents insert so all iterations miss even without flow_mask.
	 */
	{
	...
	/* TCP LRU miss counter: should match LRU misses for TCP scenarios */
...
static void xdp_lb_validate(void)
{
	...
	if (env.duration_sec != 5) {
		fprintf(stderr, "xdp-lb auto-stops when samples are collected, -d is not supported\n");
		exit(1);
	}
}

static void xdp_lb_run_once(void *arg __attribute__((unused)))
{
...
static void xdp_lb_setup(void)
{
	...
	/* Auto-stop when all samples are collected; large timeout as safety */
	...
	/* Reset counters accumulated during calibration */
